...
//         if(result.length>0)
//         {
//             console.log('WELCOME BACK -> userid: ',result[0].name,' pass: ',result[0].password);
//             res.send(result);
//         }
//         else{
...
.post("/login", (req, res) => {
    ...
    const password = req.body.password;
    const lo = 'SELECT * FROM hospital.admin WHERE Adminid = ? AND password = ?';
    const login_query = mysql.format(lo, [ID, password]);
    db.query(login_query, (err, result) => {
        if (err) {
            res.send({ err: err });
        } else {
            if (result.length > 0) {
                console.log('WELCOME BACK -> username:', result[0].name, 'pass:', result[0].password);
                res.send(result);
            } else {
                console.log('Incorrect ID or Password');
                res.send({ message: "Wrong username/password combination" });
