...
.get("/ViewAppointments/:id",(req,res)=>{
    ...
    var disp="select app.ApID,app.PID as patID,p.name as patname,date_format(app.Date, '%Y-%m-%d' ) as Date  from hospital.appointments app,hospital.patient p where app.PID = p.PID and app.DID = ? and app.Date >= curDate() ";
    var insert_q=mysql.format(disp,[ID]);
    db.query(insert_q,(err,result)=>{
        if(err) throw err;
     res.send(result);
...
.get("/ViewPres/:id",(req,res)=>{
    const ID = req.params.id
    var disp="select d.name,p.name as paname,date_format(pr.Date, '%Y-%m-%d' ) as Date,pr.MedName,pr.Dosage,pr.Diagnosis from hospital.patient p,hospital.doctors d,hospital.prescription pr  where  d.DocID=pr.DID and p.PID=pr.PID and pr.ApID = ? ";
    var insert_q=mysql.format(disp,[ID]);
    db.query(insert_q,(err,result)=>{
        if(err) throw err;
     res.send(result);
...
.get("/ViewPAppointments/:id",(req,res)=>{
    const ID = req.params.id
    var disp="select app.DeptID,app.ApID,app.PID as patID,p.name as patname,date_format(app.Date, '%Y-%m-%d' ) as Date from hospital.appointments app,hospital.patient p where app.PID = p.PID and app.DID = ? and app.Date < curDate() ";
    var insert_q=mysql.format(disp,[ID]);
    db.query(insert_q,(err,result)=>{
        if(err) throw err;
     res.send(result);
...
.post("/Prescribe",async (req,res)=>{
    ...
    await db.query(length,(err,result)=>{
      ...
      PrID="Pr"+(result.length+1);
      var insert_query = mysql.format(sqlInsert,[PrID,req.body.DID,req.body.PID,req.body.ApID,req.body.Date,req.body.MedName,req.body.Dosage,req.body.Diagnosis]);
     db.query(insert_query,(err,result)=>{
      if(err) throw err;
      else{
