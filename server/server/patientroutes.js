...
.post("/Signup",async (req,res)=>{
    ...
    // const getMaxSnoQuery = "SELECT MAX(sno) AS maxSno FROM hospital.patient";
    ...
    // var PID;
    //   await db.query(getMaxSnoQuery, (err, result) => {
    // if (err) throw err;
    // const maxSno = result[0].maxSno;
    // PID = "P" + (maxSno + 1);
    //   var SNO= maxSno+1;
    //   ID=PID;
      var insert_query = mysql.format(sqlInsert,[PID,req.body.name,req.body.age,req.body.city,req.body.phone,req.body.email_address,req.body.SEX,req.body.ADDRESS,req.body.PASSWORD]);
    // var insert_query = mysql.format(sqlInsert, [(maxSno + 1), PID, req.body.name, req.body.age, req.body.city, req.body.phone, req.body.email_address, req.body.SEX, req.body.ADDRESS, req.body.PASSWORD]);
...
.get("/ViewAppointments/:id",(req,res)=>{
    ...
    var disp="select app.ApID,d.name as docname,date_format(app.Date, '%Y-%m-%d' ) as Date  ,de.dname from hospital.appointments app,hospital.doctors d,hospital.departments de where  app.DID=d.DocID and app.DeptID=de.DID and app.PID= ?";
    var insert_q=mysql.format(disp,[ID]);
    db.query(insert_q,(err,result)=>{
        if(err) throw err;
     res.send(result);
